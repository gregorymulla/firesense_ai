...
def download_and_extract_frames(video_id, interval_seconds=1, quality="720p", output_base_dir="."):
...
def process_video_inference(video_id, interval_seconds=1, output_dir="."):
    ...
    inference_times = [r['inference_time_seconds'] for r in inference_results if r['inference_time_seconds'] > 0]
    timing_stats = {
        "total_inference_time": round(sum(inference_times), 2) if inference_times else 0,
        "average_time_per_frame": round(sum(inference_times) / len(inference_times), 3) if inference_times else 0,
    ...
    danger_count = sum(1 for r in inference_results if r['inference']['has_out_of_control_fire'])
